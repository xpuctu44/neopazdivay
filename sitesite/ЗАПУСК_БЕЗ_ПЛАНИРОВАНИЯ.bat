...
echo.
echo 5. Создаем тестового сотрудника...
python -c "
from app.database import SessionLocal
from app.models import User
from app.security import hash_password
...
echo.
echo 6. Создаем тестовый магазин...
python -c "
from app.database import SessionLocal
...
db = SessionLocal()
try:
    store = db.query(Store).filter(Store.name == 'Главный магазин').first()
    if not store:
        store = Store(
        ...
        db.add(store)
        db.commit()
        print('Магазин создан: Главный магазин')
    else:
...
echo - Вход и регистрация
echo - Dashboard с таймером
echo - Админ панель (без планирования)
